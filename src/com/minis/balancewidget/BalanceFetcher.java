...
public class BalanceFetcher {
    ...
    public static Item fetchOne(Context ctx, String keyId, int timeoutMs) {
        ...
        if (ak == null) {
            ...
            if (p0 != null && ("dashscope".equals(p0.id) || "modelscope".equals(p0.id))) {
                ...
                it.tag = p0.unit;
                it.kind = p0.kind;
                try { fill(it, "", 7.1, timeoutMs); it.ok = true; }
                catch (Exception e) {
                    it.ok = false;
    ...
    private static void fill(Item it, String key, double rate, int t) throws Exception {
        String id = it.platform;              // 解析逻辑按「平台」选，不按 Key
        boolean usd = "USD".equals(it.tag);

        /* 百炼：预付费资源包（订阅制）。
           用量与剩余额度都在控制台，DashScope 侧没有查询接口，所以只作展示。 */
